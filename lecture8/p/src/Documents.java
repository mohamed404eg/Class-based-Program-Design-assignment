...
interface ILoString{
	...
	ILoString Sort();
	// produce true if the item in the list 
	boolean Contain(String item);
	// produce ILoString without any duplicates item
	ILoString RemoveDuplicates();
	
	 ILoString RemoveDuplicatesHelper(ILoString unique);
	 // append item at the of the list
	 ILoString appendAtEnd(String item);
...
class MtLoString implements ILoString{
	...
	public ILoString RemoveDuplicatesHelper(ILoString unique) {
		return unique;
	}
	public ILoString appendAtEnd(String item) {
		return new ConsLoString(item,this);
	}
}
...
class ConsLoString implements ILoString{
	 ...
	 * ... this.insert(String item)                                 ... --- ILoString
	 * ... this.Sort()                                              ... --- ILoString
	 * ... this.Contain(String item) 								... --- boolean
	 * Methods Fields:
	 * ... this.rest.First()                                             ... --- String
	 * ... this.rest.Rest()                                              ... --- ILoString
	 * ... this.rest.AppendLoString(ILoString f1 ,ILoString f2)			 ... --- ILoString
	 ...
	 * ... this.rest.insert(String item)                                 ... --- ILoString
	 * ... this.rest.Sort()                                              ... --- ILoString
	 * ... this.rest.Contain(String item) 		                         ... --- boolean
	 * 
	 */
	public 	boolean Contain(String item){
	 ...
	 * ... this.Sort()                                              ... --- ILoString
	 * ... this.Contain(String item) 								... --- boolean
	 * ... this.RemoveDuplicates( 								    ... --- ILoString
	 * Methods Fields:
	 * ... this.rest.First()                                             ... --- String
	 * ... this.rest.Rest()                                              ... --- ILoString
	 * ... this.rest.AppendLoString(ILoString f1 ,ILoString f2)			 ... --- ILoString
	 ...
	 * ... this.rest.Sort()                                              ... --- ILoString
	 * ... this.rest.Contain(String item) 		                         ... --- boolean
	 * ... this.rest.RemoveDuplicates()								     ... --- ILoString
	 * 
	 */
	
	public ILoString RemoveDuplicates() {
		ILoString unique = new MtLoString();
		
		return this.RemoveDuplicatesHelper(unique);
		
		
	}
	
	public ILoString RemoveDuplicatesHelper(ILoString unique) {
		if (!unique.Contain(this.first)){
		return this.rest.RemoveDuplicatesHelper(unique.appendAtEnd(this.first));
		}else {
			return  this.rest.RemoveDuplicatesHelper(unique);
		}
	...
	public ILoString appendAtEnd(String item) {
		return new ConsLoString(this.first,this.rest.appendAtEnd(item));
	}
...
class ExamplesDocuments{
	...
	boolean testILoDocumentContain(Tester t) {
		return t.checkExpect(MtLoDocumebt.onlyBooks().Contain("marik, zik. 'cbook'"),false) &&
				t.checkExpect(paper1.onlyBooks().Contain("okey, mike. 'dbook'"), false) &&
				t.checkExpect(paper1.onlyBooks().Contain("ramadan, mohamed. 'book1'"), true) && 
				t.checkExpect(paper4.onlyBooks().Contain("okey, mike. 'dbook'"), true) &&
				t.checkExpect(paper4.onlyBooks().Contain("qimu, sliol. 'css'"), false);
				
	}
	
	boolean testILoDocumentRemoveDuplicates(Tester t) {
		return t.checkExpect(MtLoDocumebt.onlyBooks().RemoveDuplicates(), MtLoDocumebt.onlyBooks()) &&
				t.checkExpect(paper1.onlyBooks().RemoveDuplicates(), paper1.onlyBooks())&&
				
				t.checkExpect( new ConsLoString("marik, zik. 'cbook'",
					   new ConsLoString("okey, mike. 'dbook'",
		  			   new ConsLoString("marik, zik. 'cbook'", 
		  					   new ConsLoString("ramadan, mohamed. 'book1'",
		  							 new ConsLoString("okey, mike. 'dbook'", new MtLoString()))))).RemoveDuplicates(), 
	...
	boolean testILoDocumentAppendAtEnd(Tester t) {
			return t.checkExpect(MtLoDocumebt.onlyBooks().appendAtEnd("marik, zik. 'cbook'"),  
					new ConsLoString("marik, zik. 'cbook'", new MtLoString())) &&
				
				t.checkExpect( paper4.onlyBooks().Sort().appendAtEnd("qimu, sliol. 'css'") ,
						 new ConsLoString("marik, zik. 'cbook'",
								   new ConsLoString("okey, mike. 'dbook'",
					  			    new ConsLoString("ramadan, mohamed. 'book1'", 
					  			    		 new ConsLoString("qimu, sliol. 'css'", new MtLoString())))));
			}
